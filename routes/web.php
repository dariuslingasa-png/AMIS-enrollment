...
Route::match(['get', 'post'], '/g-callback', [GoogleAuthController::class, 'callback'])->middleware('throttle:10,1')->name('auth.google.callback');
Route::get('/auth/unsupported-browser', [GoogleAuthController::class, 'unsupportedBrowser'])->name('auth.unsupported-browser');

Route::get('/m-signin', [\App\Http\Controllers\MicrosoftAuthController::class, 'redirect'])->middleware('throttle:10,1')->name('auth.microsoft');
Route::match(['get', 'post'], '/m-callback', [\App\Http\Controllers\MicrosoftAuthController::class, 'callback'])->middleware('throttle:10,1')->name('auth.microsoft.callback');
...
Route::get('/verify-email/status', [AuthController::class, 'checkVerificationStatus'])->middleware('throttle:600,1')->name('verify.email.status');
...
Route::post('/email/verification-notification', [\App\Http\Controllers\Auth\EmailVerificationNotificationController::class, 'store'])->middleware(['auth', 'throttle:10,1'])->name('verification.send');
...
Route::middleware(['auth'])->prefix('workflow')->name('workflow.')->group(function () {
    ...
    Route::prefix('api')->name('api.')->group(function () {
        Route::get('/workflows', [WorkflowController::class, 'apiIndex'])->name('index');
        Route::post('/workflows', [WorkflowController::class, 'apiStore'])->name('store');
        Route::get('/workflows/{workflow}', [WorkflowController::class, 'apiShow'])->name('show');
        Route::patch('/workflows/{workflow}', [WorkflowController::class, 'apiUpdate'])->name('update');
        Route::delete('/workflows/{workflow}', [WorkflowController::class, 'apiDestroy'])->name('destroy');
        Route::post('/workflows/{workflow}/canvas', [WorkflowController::class, 'apiSaveCanvas'])->name('canvas');
        Route::post('/workflows/{workflow}/toggle', [WorkflowController::class, 'apiToggle'])->name('toggle');
        Route::post('/workflows/{workflow}/test-run', [WorkflowController::class, 'apiTestRun'])->name('test-run');
